...
void parseLine(char *line, Instruction *instr) {
    ...
    } else if (strcmp(inst, "ADDI") == 0 || strcmp(inst, "ANDI") == 0 || strcmp(inst, "ORI") == 0 || strcmp(inst, "XORI") == 0 ||
...
unsigned int convertInstruction(Instruction *instr) {
    ...
    // 명령어 추가 필요, 대소문자 구분 안되게 조정 필요, exit은 이미 앞에서 필터링 했음.
    return machinecode;
}
...
int main(void) {
    ...
    while (1) {
        ...
        while (fgets(line, sizeof(line), inputFile)) {
            ...
            if (instr.label[0] != '\0') continue; // label인 경우에는 continue
            ...
            if (strcmp(instr.instruction, "EXIT") == 0) {
                ...
                // instr.offset = (labelAddress - pc) / 4; offset은 어차피 binaryCode에 들어가는 것이 아니라서 의미가 없음
            }
